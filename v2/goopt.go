// Copyright 2024, Florent Heyworth. All rights reserved.
// Use of this source code is governed by the MIT licensee
// which can be found in the LICENSE file.

// Package v2 goopt provides support for command-line processing.
//
...
import (
	"fmt"
	"github.com/napalu/goopt/v2/parse"
...
// New convenience initialization method. Use NewCmdLine to
// configure CmdLine using option functions.
func New() *CmdLine {
	return NewWithRegistry(newDefaultRegistry(false))
}

func NewWithRegistry(registry Registry) *CmdLine {
	return &CmdLine{
...
// NewFromStruct parses a struct and binds its fields to command-line flags
func NewFromStruct[T any](structWithTags *T) (*CmdLine, error) {
	return NewFromStructWithLevel(structWithTags, 5)
}

// NewFromStructWithLevel parses a struct and binds its fields to command-line flags up to maxDepth levels
func NewFromStructWithLevel[T any](structWithTags *T, maxDepth int) (*CmdLine, error) {
...
func (s *CmdLine) SetExecOnParse(value bool) {
...
func (s *CmdLine) SetEnvFilter(env EnvFunc) EnvFunc {
...
func (s *CmdLine) ExecuteCommand() error {
...
func (s *CmdLine) GetCommandExecutionError(commandName string) error {
...
func (s *CmdLine) AddFlagPreValidationFilter(flag string, proc FilterFunc) error {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return fmt.Errorf("%w: (%w) %s", ErrFlagNotFound, err, flag)
	}
...
func (s *CmdLine) AddFlagPostValidationFilter(flag string, proc FilterFunc) error {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return fmt.Errorf("%w: (%w) %s", ErrFlagNotFound, err, flag)
	}
...
func (s *CmdLine) HasPreValidationFilter(flag string) bool {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return false
	}
...
// acceptable values
func (s *CmdLine) GetPreValidationFilter(flag string) (FilterFunc, error) {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return nil, fmt.Errorf("%w (%w): no pre-validation filters for flag %s", ErrValidationFailed, err, flag)
	}
...
func (s *CmdLine) HasPostValidationFilter(flag string) bool {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return false
	}
...
// acceptable values
func (s *CmdLine) GetPostValidationFilter(flag string) (FilterFunc, error) {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return nil, fmt.Errorf("%w (%w): no post-validation filters for flag %s", ErrValidationFailed, err, flag)
	}
...
func (s *CmdLine) HasAcceptedValues(flag string) bool {
	flagInfo, err := s.registry.GetArgument(flag)
	if err != nil {
		return false
	}
...
func (s *CmdLine) AddCommand(cmd *Command) error {
	return s.registry.AddCommand(cmd)
}
...
func (s *CmdLine) Parse(args []string) bool {
	s.ensureInit()
...
func (s *CmdLine) ParseString(argString string) bool {
...
func (s *CmdLine) ParseWithDefaults(defaults map[string]string, args []string) bool {
	argLen := len(args)
...
func (s *CmdLine) ParseStringWithDefaults(defaults map[string]string, argString string) bool {
	args, err := parse.Split(argString)
	if err != nil {
		return false
	}
...
func (s *CmdLine) SetPosix(posixCompatible bool) bool {
...
func (s *CmdLine) GetOrDefault(flag string, defaultValue string, commandPath ...string) string {
	flagValue, err := s.registry.GetFlagValue(flag, commandPath...)
	if err != nil {
		return defaultValue
	}
...
func (s *CmdLine) Get(flag string, commandPath ...string) (string, bool) {
	flagValue, err := s.registry.GetFlagValue(flag, commandPath...)
	if err != nil {
		return "", false
	}
...
func (s *CmdLine) GetBool(flag string, commandPath ...string) (bool, error) {
...
func (s *CmdLine) GetInt(flag string, bitSize int, commandPath ...string) (int64, error) {
	value, success := s.Get(flag, commandPath...)
	if !success {
...
func (s *CmdLine) GetFloat(flag string, bitSize int, commandPath ...string) (float64, error) {
	value, success := s.Get(flag, commandPath...)
	if !success {
		return 0, fmt.Errorf("no option with flag '%s' exists", flag)
	}
...
func (s *CmdLine) GetList(flag string, commandPath ...string) ([]string, error) {
	flagInfo, err := s.registry.GetArgument(flag, commandPath...)
	listDelimFunc := s.getListDelimiterFunc()
	if err == nil {
		if flagInfo.Argument.TypeOf == Chained {
...
func (s *CmdLine) SetListDelimiterFunc(delimiterFunc ListDelimiterFunc) error {
...
func (s *CmdLine) SetArgumentPrefixes(prefixes []rune) error {
...
func (s *CmdLine) GetConsistencyWarnings() []string {
...
func (s *CmdLine) GetWarnings() []string {
	var warnings []string
	for opt := range s.options {
		flagInfo, err := s.registry.GetArgument(opt)
		if err != nil {
			continue
		}
		...
		for _, k := range flagInfo.Argument.DependsOn {
			depInfo, err := s.registry.GetArgument(k)
			if err != nil {
				warnings = append(warnings,
					fmt.Sprintf("Flag '%s' depends on '%s' which was not specified.", flagInfo.MainKey, k))
				continue
			}
			...
			dependValue, err := s.registry.GetFlagValue(depInfo.MainKey)
			if err != nil {
				continue
			}
			...
			if !valueFound {
				warnings = append(warnings, fmt.Sprintf(
					...
					opt, depInfo.MainKey, showDependencies(flagInfo.Argument.OfValue), dependValue))
			}
		}
	}
...
func (s *CmdLine) GetOptions() []KeyValue {
...
// AddFlag is used to define a Flag. A Flag represents a command line option with a "long" name
// and an optional "short" form. By default, the long and short forms are prefixed by '-'  or '--'.
// This version supports both global flags and command-specific flags using the optional commandPath argument.
func (s *CmdLine) AddFlag(flag string, argument *Argument, commandPath ...string) error {
	return s.registry.AddFlag(flag, argument, commandPath...)
}

// BindFlagToCmdLine is a helper function to allow passing generics to the CmdLine.BindFlag method
func BindFlagToCmdLine[T Bindable](s *CmdLine, data *T, flag string, argument *Argument, commandPath ...string) error {
	if s == nil {
		return fmt.Errorf("can't bind flag to nil CmdLine pointer")
	}

	return s.registry.BindFlag(flag, argument, data, commandPath...)
}

// CustomBindFlagToCmdLine is a helper function to allow passing generics to the CmdLine.CustomBindFlag method
func CustomBindFlagToCmdLine[T any](s *CmdLine, data *T, proc ValueSetFunc, flag string, argument *Argument, commandPath ...string) error {
	if s == nil {
		return fmt.Errorf("can't bind flag to nil CmdLine pointer")
	}
...
func (s *CmdLine) BindFlag(bindPtr interface{}, flag string, argument *Argument, commandPath ...string) error {
	if bindPtr == nil {
		return fmt.Errorf("can't bind flag to nil CmdLine pointer")
	}
...
func (s *CmdLine) CustomBindFlag(data any, proc ValueSetFunc, flag string, argument *Argument, commandPath ...string) error {
...
func (s *CmdLine) AcceptPattern(flag string, val PatternValue, commandPath ...string) error {
...
func (s *CmdLine) AcceptPatterns(flag string, acceptVal []PatternValue, commandPath ...string) error {
	flagInfo, err := s.registry.GetArgument(flag, commandPath...)
	if err != nil {
		return err
	}
	...
	if flagInfo.Argument.AcceptedValues == nil {
		flagInfo.Argument.AcceptedValues = make([]LiterateRegex, 0, lenValues)
	}

	for i := 0; i < lenValues; i++ {
		if err := flagInfo.Argument.accept(acceptVal[i]); err != nil {
			return *err
		}
	}
...
func (s *CmdLine) GetAcceptPatterns(flag string, commandPath ...string) ([]LiterateRegex, error) {
	flagInfo, err := s.registry.GetArgument(flag, commandPath...)
	if err != nil {
	...
	return flagInfo.Argument.AcceptedValues, nil
}

// GetArgument returns the Argument corresponding to the long or short flag or an error when not found
func (s *CmdLine) GetArgument(flag string, commandPath ...string) (*Argument, error) {
...
func (s *CmdLine) SetArgument(flag string, paths []string, configs ...ConfigureArgumentFunc) error {
...
func (s *CmdLine) GetShortFlag(flag string, commandPath ...string) (string, error) {
...
func (s *CmdLine) HasFlag(flag string, commandPath ...string) bool {
	mainKey := s.flagOrShortFlag(flag, commandPath...)
...
func (s *CmdLine) HasRawFlag(flag string) bool {
...
func (s *CmdLine) HasCommand(path string) bool {
...
func (s *CmdLine) DescribeFlag(flag, description string) error {
	mainKey := s.flagOrShortFlag(flag)
	flagInfo, found := s.acceptedFlags.Get(mainKey)
	if found {
...
func (s *CmdLine) GetDescription(flag string) string {
	mainKey := s.flagOrShortFlag(flag)
	flagInfo, found := s.acceptedFlags.Get(mainKey)
	if found {
...
//	)
func (s *CmdLine) SetCommand(commandPath string, configs ...ConfigureCommandFunc) error {
...
func (s *CmdLine) SetFlag(flag, value string) error {
	mainKey := s.flagOrShortFlag(flag)
	key := ""
...
func (s *CmdLine) Remove(flag string) bool {
	mainKey := s.flagOrShortFlag(flag)
	_, found := s.options[mainKey]
	if found {
...
// to be valid.
func (s *CmdLine) DependsOnFlag(flag, dependsOn string) error {
	if flag == "" {
...
func (s *CmdLine) FlagPath(flag string) string {
...
func (s *CmdLine) DependsOnFlagValue(flag, dependsOn, ofValue string) error {
	if flag == "" {
		return fmt.Errorf("can't set dependency on empty flag")
	}
...
func (s *CmdLine) PrintUsageWithGroups(writer io.Writer) {
...
func (s *CmdLine) PrintGlobalFlags(writer io.Writer) {
	_, _ = writer.Write([]byte("\nGlobal Flags:\n\n"))
...
func (s *CmdLine) PrintCommandsWithFlags(writer io.Writer, config *PrettyPrintConfig) {
...
func (s *CmdLine) PrintCommandSpecificFlags(writer io.Writer, commandPath string, level int, config *PrettyPrintConfig) {
	hasFlags := false
	for f := s.acceptedFlags.Front(); f != nil; f = f.Next() {
...
func (s *CmdLine) PrintFlags(writer io.Writer) {
...
func (s *CmdLine) PrintCommands(writer io.Writer) {
...
func (s *CmdLine) PrintCommandsUsing(writer io.Writer, config *PrettyPrintConfig) {
	for kv := s.registeredCommands.Front(); kv != nil; kv = kv.Next() {
		if kv.Value.TopLevel {
			kv.Value.Visit(func(cmd *Command, level int) bool {
