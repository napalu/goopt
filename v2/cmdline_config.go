...
// NewCmdLine allows initialization of CmdLine using option functions. The caller should always test for error on
// return because CmdLine will be nil when an error occurs during initialization.
//
...
//					WithType(Chained))))
func NewCmdLine(configs ...ConfigureCmdLineFunc) (*CmdLine, error) {
	cmdLine := New()
...
func WithFlag(flag string, argument *Argument) ConfigureCmdLineFunc {
	return func(cmdLine *CmdLine, err *error) {
