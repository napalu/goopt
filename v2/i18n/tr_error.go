...
func (e *TrError) Format(provider MessageProvider) string {
	...
	if e.wrapped != nil {
		// Recursively check all wrapped errors for translatable ones
		var wrappedMsg string
		currentErr := e.wrapped

		for currentErr != nil {
			if te, ok := currentErr.(TranslatableError); ok {
				// Found a translatable error, format it with the same provider
				wrappedMsg = te.Format(provider)
				break
			}
		...
		// If we didn't find any translatable error, use the original wrapped error
		if wrappedMsg == "" {
